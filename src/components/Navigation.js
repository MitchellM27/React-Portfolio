...
function Navigation({currentPage, handlePageChange}) {
  return (
      <ul style={styles.navBar} className="nav">
        ...
        <li className='nav-item'><a href="#resume" onClick={() => handlePageChange('Resume')} className={currentPage === 'Resume' ? 'nav-link active' : 'nav-link'} style= {styles.navItems}>Resume</a></li>
      </ul>     
  );
}
